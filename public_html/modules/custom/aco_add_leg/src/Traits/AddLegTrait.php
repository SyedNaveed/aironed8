...
namespace Drupal\aco_add_leg\Traits;
...
trait AddLegTrait {
   ...
   * @var string
   */
  protected $tempstoreId = 'aco_add_leg.form_wizard';
   ...
   * @var string
   */
  protected $tempstoreMachineName = 'AddLegWizard';
   ...
   * @var string
   */
  protected $route = 'aco_add_leg.form_wizard';
  ...
  protected function fillValues(FormStateInterface $form_state) {
    ...
    if (!empty($this->values['departure']) && !empty($this->values['arrival']) && !empty($this->values['from'])) {
      ...
      if (method_exists($this, 'getQuote')) {
        ...
        $this->subtotal = $subtotal + $this->values['ancillaryPurchasesTotal'];
  ...
  protected function getChargeTables(array &$form) {
    ...
    $form['charges_summary'] = [
      ...
      '#rows' => [
        [
          'description' => $this->t('Add Leg Charge'),
